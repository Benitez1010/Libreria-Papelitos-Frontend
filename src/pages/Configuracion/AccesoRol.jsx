...
const AccesoRol = () => {
  ...
  // ESTADO ACTUALIZADO: Solo módulos de Gestión Administrativa
    ...
    <Box sx={{ mb: 3 }}>
      <Paper elevation={1} sx={{ p: 3, borderRadius: 2, border: '1px solid #e0e0e0' }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Switch color="success" checked={config[moduloKey]?.master || false} onChange={() => toggleMaster(moduloKey)} />
        </Box>
          ...
          <Stack spacing={1.5} sx={{ mt: 2, pt: 2, borderTop: '1px dashed #d0d0d0' }}>
            {actions.map((action) => (
              <Box key={action.key} sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Switch size="medium" color="success" checked={config[moduloKey][action.key] || false} onChange={() => toggleSub(moduloKey, action.key)} />
              </Box>
            ))}
          </Stack>
        )}
        ...
        <Box sx={{ maxWidth: 800, mx: 'auto' }}>
          <Box sx={{ mb: alerta.mostrar ? 2 : 0, textAlign: 'center', position: 'relative' }}>
            ...
            <Typography variant="body1" color="text.secondary">
              Administrando privilegios para: <strong>{usuario?.username || 'Cargando...'} ({usuario?.rol_display || 'Operador'})</strong>
            </Typography>
          </Box>
          {alerta.mostrar && (
